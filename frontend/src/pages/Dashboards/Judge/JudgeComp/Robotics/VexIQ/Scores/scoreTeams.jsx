...
const ScoreTeams = ({ onCalculate, onClose, gameId, eventName, eventId }) => {
  ...
  const score = useMemo(() => {
    return (
      goalCount * 1 + switchCount * 1 + passCount * getPassPoints(switchCount)
    );
          ...
          {[
            {
              label: "Switch",
              count: switchCount,
              setCount: setSwitchCount,
              icon: <FaRedo size={20} className="text-blue-500" />,
              max: 4,
            },
            {
              label: "Goal",
              count: goalCount,
              setCount: setGoalCount,
              icon: <FaBullseye size={20} className="text-green-500" />,
            },
            {
              label: "Pass",
              count: passCount,
              setCount: setPassCount,
              icon: (
                <GiThreeBurningBalls size={20} className="text-orange-500" />
              ),
            },
