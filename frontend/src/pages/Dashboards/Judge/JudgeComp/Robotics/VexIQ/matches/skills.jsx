...
import useSchedulesBatch from "../../../../../../../hooks/Schedule/useSchedulesBatch";
...
const Skills = () => {
  ...
  const activeSchedules =
    activeTab === "driver_iq" ? driverSchedules || [] : autoSchedules || [];
  ...
  const roundSchedules = roundDetails;

  // Calculate allowed rounds
  const currentChallengeCompleted = completedRounds[activeTab] || [];
  const maxCompleted =
    currentChallengeCompleted.length > 0
      ? Math.max(...currentChallengeCompleted)
      : 0;
  const nextAllowedRound = maxCompleted + 1;

  const isRoundCompleted = () => {
    const currentSchedule =
      roundSchedules?.[selectedRound - 1]?.schedule?.games;
  ...
  const Th = ({ children, className }) => (
    <th
      className={`px-2.5 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider text-center ${className}`}
    >
      {children}
    </th>
  );
  ...
  const Td = ({ children, className }) => (
    <td
      className={`px-2.5 py-2 sm:px-4 sm:py-3 text-sm text-gray-900 text-center ${className}`}
    >
      {children}
    </td>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 transition-all duration-300">
      ...
      {(driverLoading || autoLoading || scoresLoading || detailsLoading) && (
        <div className="p-6 space-y-4">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="h-14 bg-gradient-to-r from-gray-100 to-gray-200 animate-pulse rounded-lg"
            />
          ))}
        </div>
      )}
      ...
      {showTimeSelector && (
        <EditTimeIQ id={event_id} onTimeChange={handleTimeChange} />
      )}
      {/* Matches Table */}
      <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
        <table className="w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {(roundSchedules?.[selectedRound - 1]?.schedule?.games || []).map(
              (match) => {
                        ...
                        <button
                          onClick={() =>
                            openCalculator(
                              match,
                              selectedRound,
                              activeTab === "driver_iq" ? "driver" : "auto",
                            )
                          }
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-all duration-150 ${
                        ...
                        rank === 1
                          ? "from-yellow-50/50 to-yellow-50"
                          : rank === 2
                            ? "from-gray-50/50 to-gray-50"
                            : rank === 3
                              ? "from-amber-50/50 to-amber-50"
                              : ""
                      }`}
                    >
