...
const Sheet123 = () => {
  ...
  const generateTeamPDF = (team) => {
    ...
    // Add title
    doc.setFontSize(18);
    doc.text(`VEX123 Robotics Performance Report`, 105, 15, null, null, 'center');
    doc.setFontSize(14);
    doc.text(`Competition: ${currentCompetition}`, 105, 25, null, null, 'center');
    
    // Team information
    doc.setFontSize(16);
    doc.text(`Team: ${team.name}`, 20, 40);
    doc.setFontSize(12);
    
    const { totalScore, totalTime } = calculateTeamTotals(team.id);
    doc.text(`Total Score: ${totalScore}`, 20, 50);
    doc.text(`Total Time: ${formatTime(totalTime)}`, 20, 60);
    
    // Add mode details
    let yPosition = 80;
    doc.setFontSize(14);
    doc.text('Game Mode Details:', 20, yPosition);
    yPosition += 10;
    ...
    GAME_MODES.forEach(mode => {
      ...
      if (modeScore > 0 || modeTime > 0) {
        doc.setFontSize(12);
        doc.text(`Mode: ${mode.name}`, 25, yPosition);
        doc.text(`Score: ${modeScore}`, 80, yPosition);
        doc.text(`Time: ${formatTime(modeTime)}`, 120, yPosition);
        yPosition += 10;
        
        // Add missions if available
        mode.missions.forEach((mission, index) => {
          doc.text(`- ${mission.step}: ${mission.points} pts`, 30, yPosition);
          yPosition += 7;
        });
        yPosition += 5;
      }
          ...
          <div className="mt-4 p-3 bg-white rounded-lg shadow">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={() => generateTeamPDF(selectedTeam)}
