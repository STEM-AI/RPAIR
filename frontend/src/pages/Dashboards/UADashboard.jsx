...
function UADashboard() {
  ...
  const handleNavigation = (kit) => {
    ...
    navigate(`/Dashboard/Event/${snakeCaseName}`, {
      state: { competition_name: kit.name, userRole },
    });
  };
        ...
        {roboticsKits.map((kit) => (
          ...
          >
            <img
              src={kit.logo}
              alt={`${kit.name} Logo`}
              className="mb-3 w-60 h-32 object-contain"
            />
            <img
              src={kit.image}
              alt={kit.name}
            ...
            />
            <button className="mt-auto bg-white text-gray-800 font-semibold py-2 px-4 rounded-lg shadow-md transition duration-300 hover:bg-gray-200">
              View Events
            </button>
          </div>
        ))}
