...
const QuestionsNav = ({ allQuestions, currentQuestionIndex, answeredQuestions, selectedOptions, onSelect }) => (
  <ul className="flex lg:flex-col gap-2 flex-wrap lg:space-y-2">
    {allQuestions.map((q, i) => (
      <li
        key={q.id}
        onClick={() => onSelect(i)}
        className={`cursor-pointer px-3 py-2 rounded-lg transition-all flex items-center justify-center lg:justify-start text-sm relative ${
          i === currentQuestionIndex
            ...
            : answeredQuestions[q.id]
              ...
              : selectedOptions[q.id]
                ...
                : 'bg-gray-200 hover:bg-cyan-600 hover:text-white'
        }`}
      >
        <span>{i + 1}</span>
        {answeredQuestions[q.id] && (
          <span className="ml-2 text-xs bg-white text-cyan-700 px-1 rounded hidden lg:inline">Answered</span>
        )}
      </li>
    ))}
  </ul>
...
// ✅ مكون Sidebar (ديسكتوب ثابت + Drawer في الموبايل)
const Sidebar = ({ show, onClose, logoSrc, timer, navComponent, answeredCount, total }) => (
  <motion.div
    initial={{ x: '-100%' }}
    animate={{ x: show ? 0 : '-100%' }}
    transition={{ duration: 0.3 }}
    className="fixed lg:static top-0 left-0 w-64 bg-cyan-700 text-white flex flex-col h-full z-50 lg:translate-x-0"
  >
    {/* Header */}
    <div className="p-6 pb-4 border-b border-cyan-600 flex justify-between items-center">
      <img src={logoSrc} alt="Logo" className="w-28" />
      <button onClick={onClose} className="lg:hidden text-white text-xl">&times;</button>
    </div>

    {/* Timer */}
    <div className="flex justify-center p-4 border-b border-cyan-600">
      {timer}
    </div>
...
const CompetitionQuestions = () => {
  const { competition, game_id } = useParams();
  ...
  const { submitGame, isSubmitting } = useSubmitGame(game_id);
  ...
  const saveAnswer = useCallback(async (questionId, answerId) => {
    ...
    try {
      ...
      await axios.post(
        ...
        { question_id: questionId, answer_id: answerId, event_game_id: game_id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setSavedAnswers(prev => ({ ...prev, [questionId]: true }));
    } catch (error) {
      Swal.fire({ icon: 'error', title: 'Save Error', text: error.response?.data?.error || 'Failed to save your answer.' });
    } finally {
      setIsSaving(false);
    }
  }, [savedAnswers, token]);
  ...
  const handleTimeUp = useCallback(async () => {
    const unsaved = Object.entries(selectedOptions).filter(([id, ans]) =>
      ans !== null && !savedAnswers[id] && (Array.isArray(ans) ? ans.length > 0 : true)
    );
    try {
      await Promise.all(unsaved.map(([id, ans]) => saveAnswer(id, ans)));
      await submitGame(game_id);
      Swal.fire({ icon: 'error', title: 'Time\'s up!', text: 'The competition has ended.' })
        .then(() => navigate(`/competition/programming/${competition}/${competition_id}`, { replace: true }));
    } catch {
      Swal.fire({ icon: 'error', title: 'Submission Error', text: 'Failed to submit your answers.' });
    }
  }, [selectedOptions, savedAnswers, saveAnswer]);
  ...
  useEffect(() => {
    if (allQuestions?.length > 0 && !currentQuestionId) {
      ...
      const init = {};
      allQuestions.forEach(q => init[q.id] = q.type === 'multiple' ? [] : null);
  ...
  const handleOptionSelect = (qid, oid, multi) => {
    setSelectedOptions(prev => multi
      ? { ...prev, [qid]: prev[qid]?.includes(oid) ? prev[qid].filter(id => id !== oid) : [...prev[qid], oid] }
      : { ...prev, [qid]: oid });
  };

  const answerDone = async () => {
    const currentId = allQuestions[currentQuestionIndex].id;
    const ans = selectedOptions[currentId];
    if (!ans || (Array.isArray(ans) && ans.length === 0)) {
      Swal.fire({ icon: 'warning', title: 'No Answer Selected', text: 'Please select an answer before submitting.' });
      return;
    }
    await saveAnswer(currentId, ans);
    setAnsweredQuestions(prev => ({ ...prev, [currentId]: true }));
    if (currentQuestionIndex < allQuestions.length - 1) {
      setCurrentQuestionIndex(i => i + 1);
    } else {
      submitGame();
      Swal.fire({ title: 'Done!', text: 'Thank you, answers submitted.', icon: 'success' })
        .then(() => navigate(`/competition/programming/${competition}/${competition_id}`, { replace: true }));
    }
  };

  const handleNext = () => {
    if (currentQuestionIndex < allQuestions.length - 1 && !isSaving) {
      setCurrentQuestionIndex(i => i + 1);
    }
  };
  ...
  const handlePrev = () => {
    if (currentQuestionIndex > 0) {
      ...
      if (!answeredQuestions[prevId]) {
        setCurrentQuestionIndex(i => i - 1);
      } else {
        Swal.fire({ icon: 'info', title: 'Already Answered', text: 'You cannot return to this question.' });
      }
    }
  };
  ...
  const handleQuestionSelect = (i) => {
    ...
    if (!answeredQuestions[qid]) setCurrentQuestionIndex(i);
    else Swal.fire({ icon: 'info', title: 'Already Answered', text: 'You cannot return to this question.' });
  };

  // Loading / Error states
  if (allLoading || detailLoading) return <div className="flex justify-center items-center h-screen"><div className="animate-spin h-16 w-16 border-t-2 border-b-2 border-cyan-600 rounded-full"></div></div>;
  if (allError) return <div>Error loading questions</div>;
  if (detailError) return <div>Error loading question</div>;
    ...
    <div className="flex">
      ...
      <div className="hidden lg:block">
        <Sidebar
          ...
          onClose={() => {}}
          logoSrc={logo}
          timer={<CircularTimer duration={infoQuestions.time_limit} onEnd={handleTimeUp} />}
          navComponent={<QuestionsNav allQuestions={allQuestions} currentQuestionIndex={currentQuestionIndex} answeredQuestions={answeredQuestions} selectedOptions={selectedOptions} onSelect={handleQuestionSelect} />}
          answeredCount={Object.values(selectedOptions).filter(opt => opt !== null && (!Array.isArray(opt) || opt.length > 0)).length}
      ...
      <Sidebar
        ...
        total={allQuestions.length}
      />
      <button onClick={() => setSidebarOpen(true)} className="lg:hidden fixed top-4 left-4 bg-cyan-600 text-white p-2 rounded z-50">☰</button>
      ...
      <div className="flex-1 p-4">
        {detailedQuestion && (
          <AnimatePresence mode="wait">
            ...
            >
              <h2 className="text-lg md:text-xl font-semibold mb-4">{currentQuestionIndex + 1}. {formattedQuestion}</h2>

              {formattedCode && (
                <div className="mb-6 overflow-x-auto text-xs md:text-sm">
                  <SyntaxHighlighter language="python" style={tomorrow} showLineNumbers wrapLines>
                    {formattedCode}
                  </SyntaxHighlighter>
                </div>
              )}
              ...
              <div className="space-y-2">
                {detailedQuestion.answers?.map(option => (
                  <div
                    key={option.id}
                    ...
                    className={`p-3 border rounded-lg cursor-pointer ${
                      ...
                      (Array.isArray(selectedOptions[detailedQuestion.id]) && selectedOptions[detailedQuestion.id].includes(option.id))
              ...
              <div className="flex justify-between mt-6">
                <button onClick={handlePrev} disabled={currentQuestionIndex === 0} className="px-4 py-2 bg-gray-500 text-white rounded disabled:opacity-50">Previous</button>
                <button onClick={answerDone} disabled={isSaving} className="px-4 py-2 bg-green-600 text-white rounded disabled:opacity-50">
                  {currentQuestionIndex < allQuestions.length - 1 ? 'Done' : 'Submit'}
                </button>
                {currentQuestionIndex < allQuestions.length - 1 && (
                  <button onClick={handleNext} className="px-4 py-2 bg-cyan-600 text-white rounded">Next</button>
                )}
              </div>
            </motion.div>
          </AnimatePresence>
