...
const CompetitionQuestions = () => {
  ...
  const { questions: infoQuestions } = useInfoQuestions(
    competition_id,
    competition,
  );
  ...
  const {
    ...
    loading: allLoading,
    error: allError,
  } = useAllQuestion(competition_id, competition);
          ...
          <div className="p-4 border-t border-cyan-600 text-center text-sm">
            {
              Object.values(selectedOptions).filter(
                (opt) =>
                  opt !== null && (!Array.isArray(opt) || opt.length > 0),
              ).length
            }
